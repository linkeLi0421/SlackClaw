...
| `decider.py` | Parses trigger type (prefix/mention) and command kind (SHELL/KIMI/CODEX/CLAUDE/FILE/MEMORY) |
| `executor.py` | Runs shell commands via subprocess or invokes agent CLIs with thread context and memory injection |
| `memory.py` | Persistent memory system: Markdown file storage, FTS5 search, prompt injection, auto-extraction |
...
| `state_store.py` | SQLite persistence (WAL mode) — tasks, approvals, sessions, thread context, checkpoints, memories (FTS5) |
...
| `models.py` | Frozen dataclasses: TaskSpec, TaskStatus, SlackMessage, MemoryRecord, etc. |
...
**Key defaults:** `DRY_RUN=true`, `LISTENER_MODE=socket`, `APPROVAL_MODE=reaction`, `WORKER_PROCESSES=1`, `EXEC_TIMEOUT_SECONDS=120`, `FILE_OUTPUT_THRESHOLD=4000`, `MEMORY_ENABLED=false`

**Memory system:** When `MEMORY_ENABLED=true`, memories are stored as `.md` files in `~/.config/SlackClaw/memory/` (platform-appropriate) and indexed in the `memories` + `memories_fts` (FTS5) tables. Config keys: `MEMORY_MAX_PER_SCOPE`, `MEMORY_RETENTION_DAYS`, `MEMORY_INJECTION_MAX_CHARS`, `MEMORY_AUTO_EXTRACT`, `MEMORY_DIR`.
